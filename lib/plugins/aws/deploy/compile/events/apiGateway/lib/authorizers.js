  ...
  compileAuthorizers() {
    _.forEach(this.serverless.service.functions, (functionObject, functionName) => {
      functionObject.events.forEach(event => {
        ...
        var tmpFunc = {};
        this.serverless.cli.log('AUTHORIZE :' + normalizedAuthorizerName);
        var tmpFu = `${normalizedAuthorizerName}ApiGatewayAuthorizer`;//normalizedAuthorizerName.toLowerCase();
        this.serverless.cli.log('LOGICB :' + tmpFu);
        tmpFunc[tmpFu] = [ authorizerObject ];
        _.merge(this.serverless.service.provider.compiledCloudFormationTemplate.TOSPLIT_ELEMENTS,
              tmpFunc);

        tmpFunc = {};
        this.serverless.cli.log('AUTHORIZE :' + normalizedAuthorizerName);
        var tmpFu = normalizedAuthorizerName.toLowerCase();
        this.serverless.cli.log('LOGICB :' + tmpFu);
        tmpFunc[tmpFu] = [ authorizerObject ];
        _.merge(this.serverless.service.provider.compiledCloudFormationTemplate.TOSPLIT_ELEMENTS,
              tmpFunc);
...
/*
        var tmpParams = {};
        var apiName = `${normalizedAuthorizerName}ApiGatewayAuthorizer`;
        tmpParams[apiName] = { "Fn::GetAtt": [ tmpFu, "Outputs." + apiName  ]};
